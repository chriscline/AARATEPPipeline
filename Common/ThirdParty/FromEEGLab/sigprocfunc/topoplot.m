...
HLINEWIDTH = 1.5;         % default linewidth for head, nose, ears
...
newCLims = [amin+cax_sgn(1)*(0.05*abs(amin)) amax+cax_sgn(2)*(0.05*abs(amax))];
if diff(newCLims)==0
	newCLims(2) = newCLims(2) + 1;
end
caxis(newCLims);   % Adding 5% to the color limits
...
if strcmp(CONVHULL,'on') %%%%%%%%% mask outside the convex hull of the electrodes %%%%%%%%%
  ...
  xx = xx*1.04; yy = yy*1.04;           % extend spline outside electrode marks
...
if ~isstr(HEADCOLOR) | ~strcmpi(HEADCOLOR,'none')
   ...
   ringh = patch([headx, NaN],[heady, NaN],ones(1,length(heady)+1),'k','EdgeColor',HEADCOLOR,'LineWidth',HLINEWIDTH);
end
        ...
        for kk = 1:length(1:length(x))
            if abs(Values(kk))
                if strcmpi(PLOTDISK, 'on')
                    ...
                    hp2 = disk(y(kk),x(kk),0.03, tmpcolor, 0, 360, 100);
                end;
            end;
        end
    end
