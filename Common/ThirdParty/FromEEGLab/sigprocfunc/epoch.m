...
for index = 1:length(events)
   ...
   elseif 1
	   if length(events)==size(data,3)
		   % assume previously epoched, and 1 event per epoch at time 0
		   pos0 = events(1);
		   posinit = pos0+reallim(1);
		   posend = pos0+reallim(2);
		   assert(posinit > 0);
		   assert(posend < size(data,2));
		   tmpdata = data(:,posinit:posend,index);
		   assert(all(isinf(g.valuelim)));
		   epochdat(:,:,index) = tmpdata;
		   indexes(index)=1;
