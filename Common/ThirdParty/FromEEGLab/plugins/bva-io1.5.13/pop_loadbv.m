...
if ~useMetadataForFilenames
	% use actual filename rather than name in header
	[~, filename, ext] = fileparts(hdrfile);
	dataFilename = [filename '.eeg'];
else
	dataFilename = conf.commoninfos.datafile;
end

EEG.comments = ['Original file: ' dataFilename];
...
[IN, message] = fopen(fullfile(path, dataFilename), 'r');
if IN == -1
    [IN, message] = fopen(fullfile(path, lower(dataFilename)));
    if IN == -1
        error(message)
    end;
end
...
if isfield(hdr.commoninfos, 'markerfile')
	...
	if ~useMetadataForFilenames
		% use actual filename rather than name in header
		[~, filename, ext] = fileparts(hdrfile);
		markerFilename = [filename '.vmrk'];
	else
		markerFilename = conf.commoninfos.markerfile;
	end
    MRK = readbvconf(path, markerFilename);
