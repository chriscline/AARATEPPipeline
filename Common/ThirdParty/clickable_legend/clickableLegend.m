...
assert(all(ishandle(plotObj(:))) || isa(plotObj,'handle'));
% convert any double handles to object handles
if isnumeric(plotObj)
	plotObj = handle(plotObj);
end
...
for i = 1:length(plothan)
	btnDownFcn = @(h,e) toggleOrHighlight(h,e,...
		{objhan(i),plotObj(groupmem==groups(i))},...
		{objhan(i),plothan(i),plotObj(groupmem==groups(i)),plotOptions});
	
    set(objhan(i), 'HitTest', 'on', 'ButtonDownFcn',btnDownFcn);
	objhan(i).UserData.IsVisible = true;
	
    if ~isempty(dispinds) && hidden(i)
        togglevisibility(objhan(i), plotObj(groupmem==groups(i)));
    end
    set(plotObj(groupmem==groups(i)), 'HitTest', 'on', 'ButtonDownFcn', btnDownFcn);
	plotObj(groupmem==groups(i)) = c_setField(plotObj(groupmem==groups(i)),'UserData.IsHighlighted',false);
...
function toggleOrHighlight(h,e,visArgs,hiArgs)
	if e.Button == 3
		togglevisibility(visArgs{:});
	else
		highlightObject(hiArgs{:});
	end


function togglevisibility(hObject, obj)
if hObject.UserData.IsVisible % It is on, turn it off
    set(hObject, 'Color', (get(hObject, 'Color') + 1)/1.5);
    set(obj,'HitTest','off','Visible','off','handlevisibility','off');
else
    set(hObject, 'Color', get(hObject, 'Color')*1.5 - 1);
    set(obj, 'HitTest','on','visible','on','handlevisibility','on');
end
hObject.UserData.IsVisible = ~hObject.UserData.IsVisible;

function highlightObject(lTextObj, lMarkerObj, plotObj, plotOptions)
lw = get(plotObj,'LineWidth');
if ~iscell(lw), lw = {lw}; end;
hasMarkerSize = isprop(plotObj,'MarkerSize');
if hasMarkerSize	
	ms = get(plotObj,'MarkerSize');
	if ~iscell(ms), ms = {ms}; end;
end

if ~plotObj(1).UserData.IsHighlighted % It is not selected, highlight it
	...
	if hasMarkerSize
		...
		set(plotObj, {'LineWidth'}, cellfun(@(x)x+2, lw, 'Uniformoutput', false));
	end
	c_setField(plotObj,'UserData.IsHighlighted',true);
else
    ...
    c_setField(plotObj,'UserData.IsHighlighted',false);
end
if ~isempty(plotOptions)
