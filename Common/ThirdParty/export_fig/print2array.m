...
%}
	...
	% adjust res so that res_str works out to multiple to fix weird matlab rendering issue with scatter points
	tmp = get(0, 'ScreenPixelsPerInch')*res;
	if tmp > 100
		res = res * round(tmp,-2) / ceil(tmp) + 0.1;
	end
